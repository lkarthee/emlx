...
static ERL_NIF_TERM add(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
  ...
  res_type * other = nullptr;
  if ((other = res_type::get_resource(env, argv[1], error)) == nullptr) {
    return error;
  }

  stream_res_type * stream_res = nullptr;
  mlx_stream stream = nullptr;
  if ((stream_res = stream_res_type::get_resource(env, argv[2], error)) == nullptr) {
    stream = mlx_default_stream(mlx_default_device());
  } else {
    stream = stream_res->val;
  }
  ...
  auto op_array = res_type::allocate_resource(env, error);
  op_array->val = mlx_add(array->val, other->val, stream);
  ERL_NIF_TERM ret = op_array->make_resource(env);
  return erlang::nif::ok(env, ret);
}
